...
##------------------------------------- PARAMETERS
DISCORD_BOT_TOKEN = None #Will pull from "token.txt" in the bot directory
COMMAND_CHARACTER='tau.' #The command prefix that you want 
...
VERSION='0.1.0'
RELEASE=False #affects the level of debug output
TICKER_ENABLE=False
...
#read the bot token from a file ("token.txt") - MAKE SURE YOUR TOKEN.TXT HAS YOUR BOT'S UNIQUE TOKEN
with open('token.txt') as token_file:
	DISCORD_BOT_TOKEN = token_file.readline()
	logging.info('Using bot token: %s',DISCORD_BOT_TOKEN)
...
async def on_ready():
	...
	#configure and kick off the crypto ticker, if enabled
	if(TICKER_ENABLE):
		logging.info('\n---------------------- Begin Ticker Startup')
...
async def on_message(message):
	...
	if(message.author==client.user): return #ignore messages coming from TAU (performance)
...
async def bot_help(message):
	logging.debug('bot_help(%s)',message.content)
	
	display_string="TAU "+VERSION+" command list:\n-------------------------------------\n"
...
async def bot_status(message):
	logging.debug('bot_status(%s)',message.content)
	
	display_string = '```'+'TAU\'s status is: GOOD'+'```'
...
async def print_coin_references(message_channel,coin_ref,send_lines=15):
	...
	get_ticker_update(); #update ticker JSON data
...
async def print_full_ticker(send_lines=15,purge=True):
	...
	#purge channel of last 100 TAU messages if option is set
...
on_message_handlers = { "status" 	: bot_status,
						"alive" 	: bot_status,
						"help"		: bot_help,
						"h"			: bot_help }

on_message_helpstrings = {  "status" 	: "Displays TAU current status",
							"alive" 	: "Ping TAU to see if it's still running",
...
if(TICKER_ENABLE): client.loop.create_task(crypto_ticker_update())
